...
function popWindow(winobj, width, height) {
    ...
    $('body').append('<div class="shade" closeTag="' + objwinId + '"></div>');
    // 设置弹窗内容最大高度
    if (height) {
...
function defaultLeftNav() {
    ...
    $('.default-slide-nav-wrap').on('click', '.slide-link', function(e) {
        ...
        if ($('.slide-nav-btn').hasClass('on')) {
            $('.default-slide-nav-wrap').animate({
                width: '200px'
            }, 300);
            $('.default-right-content').animate({
                width: ($('.default-right-content').width() - 150) + 'px',
                paddingLeft: '200px'
            }, 300);
...
/* 
    ...
    author:wumj
*/
function zoomInPic(zoomInObj) {
    var small = zoomInObj.find('.small-pic').eq(0);
    var big = zoomInObj.find('.big-pic').eq(0);
    var jingtou = zoomInObj.find('.target').eq(0);
    var bigimg = zoomInObj.find('.big-pic img').eq(0);
    //监视鼠标(镜头)
    ...
    //挡鼠板移动的时候
    small.bind('mousemove', function(event) {
        //获得当前坐标 //减去镜头宽度的一半
        var left = event.pageX - zoomInObj.offset().left - jingtou.width() / 2;
        var top = event.pageY - zoomInObj.offset().top - jingtou.height() / 2;
        //向左走
        if (left <= 0) {
            left = 0;
        }
        ...
        //向右走
        if (left >= small.width() - jingtou.width()) {
            left = small.width() - jingtou.width();
        }
        //向下走
        if (top >= small.height() - jingtou.height()) {
            top = small.height() - jingtou.height();
        }
        //小图的比例
        var smallX = left / (small.width() - jingtou.width());
        var smallY = top / (small.height() - jingtou.height());
        var bigX = -smallX * (bigimg.width() - big.width());
        var bigY = -smallY * (bigimg.height() - big.height());
        bigimg.css('left', bigX + 'px');
        bigimg.css('top', bigY + 'px');
        //镜头距离左边的位置==鼠标距离左边的位置
        jingtou.css('left', left + 'px');
        jingtou.css('top', top + 'px');
    });
}